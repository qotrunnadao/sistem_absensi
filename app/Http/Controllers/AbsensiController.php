...
class AbsensiController  extends Controller
{
	...
	public function store(Request $request)
	{
		...
		if ($request->jenis == 1) {
			$cek_masuk = Absensi::whereDate('created_at', Carbon::today())->where([
				'user_id' => $absensi,
				'jenis' => 1,
			])->count();
			if ($cek_masuk > 0) {
				Alert::error('Gagal', 'Sudah absen hari ini');
				return redirect('/absensi');
			}
		} else if ($request->jenis == 2) {
			$cek_pulang = Absensi::whereDate('created_at', Carbon::today())->where([
				'user_id' => $absensi,
				'jenis' => 2,
			])->count();
			if ($cek_pulang > 0) {
	...
	public function cetak()
	{
		if (Auth::user()->level == 0) {
			$data = array(
				'data_user' => User::where('level', 1)->latest()->get(),
			);
		} else {

			$user = Auth::user()->id;
			$data = array(
				'data_user' => User::where(['id' => $user, 'level' => 1])->latest()->get(),
			);
		}
	...
	{
		if (Auth::user()->level == 0) {
			$cetakabsensi = Absensi::with('user')->where('jenis', 1)->whereBetween('created_at', [$dari, $sampai])->get();
			$izin = Izin::with('user')->where('status', 1)->whereBetween('created_at', [$dari, $sampai])->get();

			$user = User::where('level', 1)->latest()->get();
			...
			$izin = Izin::with('user')->where('user_id', $absensi)->where('status', 1)->whereBetween('created_at', [$dari, $sampai])->get();
			return view(
				'absensi.cetakabsensi',
				compact(
					'cetakabsensi',
